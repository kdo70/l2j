...
public class AdminInfo implements IAdminCommandHandler {
    private static final CLogger LOGGER = new CLogger(AdminInfo.class.getName());
                    ...
                    switch (subCommand) {
                        ...
                        case "spoil":
                            try {
                                ...
                                sendDropInfos(targetNpc, html, page, subCommand.equalsIgnoreCase("drop"), player);
                            } catch (Exception e) {
                                sendDropInfos(targetNpc, html, 1, true, player);
                            }
                            break;
     ...
     */
    private static void sendDropInfos(Npc npc, NpcHtmlMessage html, int page, boolean isDrop, Player player) {
        // Load static htm.
        html.setFile("data/html/admin/npcinfo/default.htm");
        ...
        for (DropCategory category : list) {
            double catChance = category.getChance() * (category.getDropType().getDropRate(npc.isRaidBoss()) + RateHelper.getDropCategoryRate(category.getDropType(), npc.isRaidBoss(), player));
            ...
            for (DropData drop : category.getAllDrops()) {
                double chance = drop.getChance() * (chanceMultiplier + RateHelper.getDropRate(drop, npc.isRaidBoss(), player));
                if (chance > 100) {
                    chance = 100.;
                }
                final String color = (chance > 80.) ? "90EE90" : (chance > 5.) ? "BDB76B" : "F08080";
                final String percent = PERCENT.format(chance);
                final String amount = (drop.getMinDrop() == drop.getMaxDrop()) ? (int) (drop.getMinDrop() * countMultiplier * RateHelper.getDropCountRate(drop, npc.isRaidBoss(), player)) + "" : (int) (drop.getMinDrop() * countMultiplier * RateHelper.getDropCountRate(drop, npc.isRaidBoss(), player)) + " - " + (int) (drop.getMaxDrop() * countMultiplier * RateHelper.getDropCountRate(drop, npc.isRaidBoss(), player));
