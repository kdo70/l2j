...
abstract class DocumentBase {
    ...
    protected void attachEffect(Node n, Object template, Condition attachCond) {
        ...
        if (attrs.getNamedItem("time") != null) {
            ...
            final boolean enabledSkillDuration = Config.ENABLED_SKILL_DURATION_LIST;
            final boolean needMulDuration = Config.SKILL_DURATION_LIST.containsKey(((L2Skill) template).getId());

            if (enabledSkillDuration && needMulDuration) {
                int level = ((L2Skill) template).getLevel();

                if (level < 100) {
                    time = Config.SKILL_DURATION_LIST.get(((L2Skill) template).getId());
                } else if (level < 140) {
                    time = Config.SKILL_DURATION_LIST.get(((L2Skill) template).getId()) + 90 * (level - 100);
                } else if (level > 140) {
                    time = Config.SKILL_DURATION_LIST.get(((L2Skill) template).getId());
                }
            }
