...
public final class Player extends Playable {
    ...
    public void updatePvPFlag(int value) {
        if (getPvpFlag() == value) return;

        setPvpFlag(value);
        sendPacket(new UserInfo(this));

        if (_summon != null) sendPacket(new RelationChanged(_summon, getRelation(this), false));
    ...
    public int getRelation(Player target) {
        ...
        if (getPvpFlag() != 0) result |= RelationChanged.RELATION_PVP_FLAG;
        if (getKarma() > 0) result |= RelationChanged.RELATION_HAS_KARMA;

        if (isClanLeader()) result |= RelationChanged.RELATION_LEADER;

        if (getSiegeState() != 0) {
            result |= RelationChanged.RELATION_INSIEGE;
            if (getSiegeState() != target.getSiegeState()) result |= RelationChanged.RELATION_ENEMY;
            else result |= RelationChanged.RELATION_ALLY;
        ...
        if (getClan() != null && target.getClan() != null) {
            if (target.getPledgeType() != Clan.SUBUNIT_ACADEMY && getPledgeType() != Clan.SUBUNIT_ACADEMY && target.getClan().isAtWarWith(getClan().getClanId())) {
                ...
                if (getClan().isAtWarWith(target.getClan().getClanId())) result |= RelationChanged.RELATION_MUTUAL_WAR;
            }
        }
        return result;
    }
    ...
    public void revalidateZone(boolean force) {
        ...
        if (Config.ALLOW_WATER) {
            if (isInWater()) WaterTaskManager.getInstance().add(this);
            else WaterTaskManager.getInstance().remove(this);
        }
    ...
    public void refreshWeightPenalty() {
        ...
        if (ratio < 0.5) newWeightPenalty = WeightPenalty.NONE;
        else if (ratio < 0.666) newWeightPenalty = WeightPenalty.LEVEL_1;
        else if (ratio < 0.8) newWeightPenalty = WeightPenalty.LEVEL_2;
        else if (ratio < 1) newWeightPenalty = WeightPenalty.LEVEL_3;
        else newWeightPenalty = WeightPenalty.LEVEL_4;
    ...
    public void refreshExpertisePenalty() {
        ...
        for (final ItemInstance item : getInventory().getPaperdollItems()) {
            if (item.getItemType() != EtcItemType.ARROW && item.getItem().getCrystalType().getId() > expertiseLevel) {
                ...
                else armorPenalty += (item.getItem().getBodyPart() == Item.SLOT_FULL_ARMOR) ? 2 : 1;
            }
        }
        ...
        if (_weaponGradePenalty != weaponPenalty || _armorGradePenalty != armorPenalty) {
            ...
            if (item != null) {
                if (_weaponGradePenalty) ItemPassiveSkillsListener.getInstance().onUnequip(Paperdoll.NULL, item, this);
                else ItemPassiveSkillsListener.getInstance().onEquip(Paperdoll.NULL, item, this);
            }
        }
    }
    ...
    public void useEquippableItem(ItemInstance item, boolean abortAttack) {
        ...
        if (isEquipped) {
            ...
            else sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_DISARMED).addItemName(item));
            ...
            if (item.isEquipped()) {
                ...
                else sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_EQUIPPED).addItemName(item));

                if ((item.getItem().getBodyPart() & Item.SLOT_ALLWEAPON) != 0) rechargeShots(true, true);
        ...
        if (abortAttack) getAttack().stop();

        if (getStatus().getInventoryLimit() != oldInvLimit) sendPacket(new ExStorageMaxCount(this));
    }
    ...
    public void setClassId(int id) {
        ...
        try {
            if (getLvlJoinedAcademy() != 0 && _clan != null && ClassId.VALUES[id].getLevel() == 2) {
                ...
                int points;
                if (getLvlJoinedAcademy() <= 16) points = 400;
                else if (getLvlJoinedAcademy() >= 39) points = 170;
                else points = 400 - (getLvlJoinedAcademy() - 16) * 10;
            ...
            if (isSubClassActive()) _subClasses.get(_classIndex).setClassId(id);

            broadcastPacket(new MagicSkillUse(this, this, 5103, 1, 1000, 0));
            setClassTemplate(id);

            if (getClassId().getLevel() == 3) sendPacket(SystemMessageId.THIRD_CLASS_TRANSFER);
            else sendPacket(SystemMessageId.CLASS_TRANSFER);

            // Update class icon in party and clan
            if (_party != null) _party.broadcastPacket(new PartySmallWindowUpdate(this));

            if (_clan != null) _clan.broadcastToMembers(new PledgeShowMemberListUpdate(this));
    ...
    public boolean sitDown() {
        ...
        if (qs != null) qs.getQuest().notifyEvent("CE8388608", null, this);
    ...
    public void addAdena(String process, int count, WorldObject reference, boolean sendMessage) {
        if (sendMessage) sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_S1_ADENA).addNumber(count));
    ...
    public boolean reduceAdena(String process, int count, WorldObject reference, boolean sendMessage) {
        ...
        if (count > 0) {
            ...
            if (!_inventory.reduceAdena(process, count, this, reference)) return false;
    ...
    public boolean reduceAncientAdena(String process, int count, WorldObject reference, boolean sendMessage) {
        ...
        if (count > 0) {
            ...
            if (!_inventory.reduceAncientAdena(process, count, this, reference)) return false;
    ...
    public boolean destroyItem(String process, ItemInstance item, int count, WorldObject reference, boolean sendMessage) {
        ...
        // Send inventory update packet
        final InventoryUpdate iu = new InventoryUpdate();
        if (item.getCount() == 0) iu.addRemovedItem(item);
        else iu.addModifiedItem(item);
        sendPacket(iu);
    ...
    public boolean destroyItemWithoutTrace(int objectId, int count) {
        final ItemInstance item = _inventory.getItemByObjectId(objectId);
        if (item == null || item.getCount() < count) return false;
    ...
    public boolean dropItem(String process, ItemInstance item, WorldObject reference, boolean sendMessage) {
        ...
        // Sends message to client if requested
        if (sendMessage) sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_DROPPED_S1).addItemName(item));
    ...
    public ItemInstance checkItemManipulation(int objectId, int count) {
        if (World.getInstance().getObject(objectId) == null) return null;
        ...
        if (_summon != null && _summon.getControlItemId() == objectId || _mountObjectId == objectId) return null;

        if (getActiveEnchantItem() != null && getActiveEnchantItem().getObjectId() == objectId) return null;

        // We cannot put a Weapon with Augmention in WH while casting (Possible Exploit)
        if (item.isAugmented() && getCast().isCastingNow()) return null;
    ...
    public void setSpawnProtection(boolean protect) {
        if (protect) {
            if (_protectTask == null) _protectTask = ThreadPool.schedule(() -> {
    ...
    public void onAction(Player player, boolean isCtrlPressed, boolean isShiftPressed) {
        if (player.getTarget() != this) player.setTarget(this);
            ...
            else if (isOperating()) player.getAI().tryToInteract(this, isCtrlPressed, isShiftPressed);
            else player.getAI().tryToFollow(this, isShiftPressed);
        }
    }

    @Override
    public void broadcastPacket(L2GameServerPacket packet, boolean selfToo) {
        if (selfToo) sendPacket(packet);
    ...
    public final void broadcastCharInfo() {
        for (final Player player : getKnownType(Player.class)) {
            ...
            if (_summon != null) player.sendPacket(new RelationChanged(_summon, relation, isAutoAttackable));
        }
    }
    ...
    public boolean canOpenPrivateStore(boolean cancelActiveTrade) {
        ...
        if (isInStoreMode()) return !isAlikeDead();

        // Cancel active trade, if parameter is on.
        if (cancelActiveTrade && getActiveTradeList() != null) cancelActiveTrade();
    ...
    public void tryOpenPrivateSellStore(boolean isPackageSale) {
        ...
        if (getOperateType() == OperateType.NONE || getOperateType() == OperateType.SELL || getOperateType() == OperateType.PACKAGE_SELL) {
            ...
            if (getOperateType() == OperateType.SELL || getOperateType() == OperateType.PACKAGE_SELL) standUp();
    ...
    public void tryOpenWorkshop(boolean isDwarven) {
        ...
        if (getOperateType() == OperateType.NONE || getOperateType() == OperateType.MANUFACTURE) {
            ...
            if (getOperateType() == OperateType.MANUFACTURE) standUp();
    ...
    public void setTarget(WorldObject newTarget) {
        ...
        if (newTarget != null) {
            ...
            if (newTarget instanceof FestivalMonster && !isFestivalParticipant()) newTarget = null;
        ...
        if (oldTarget != null) {
            ...
            if (oldTarget instanceof Creature) ((Creature) oldTarget).getStatus().removeStatusListener(this);
        }
        ...
        // Add the Player to the _statusListener of the new target if it's a Creature
            ...
            if (newTarget.getObjectId() != getObjectId()) sendPacket(new ValidateLocation(target));
        ...
        if (newTarget instanceof Folk) setCurrentFolk((Folk) newTarget);
    ...
    public void engageAnswer(int answer) {
        if (!_isUnderMarryRequest || _requesterId == 0) return;
    ...
    public boolean doDie(Creature killer) {
        ...
        if (killer != null) {
            ...
            if (isCursedWeaponEquipped()) CursedWeaponManager.getInstance().drop(_cursedWeaponEquippedId, killer);
            else {
                    ...
                    if (!isInArena()) {
                        ...
                        if (pk != null && pk.getClan() != null && getClan() != null && !isAcademyMember() && !pk.isAcademyMember()) {
                            ...
                            if (_clan.isAtWarWith(pk.getClanId()) && pk.getClan().isAtWarWith(_clan.getClanId())) {
                                ...
                                if (getClan().getReputationScore() > 0) pk.getClan().addReputationScore(1);
                                // when the opposing sides reputation score is 0 or below, your clans reputation score doesn't decrease
                                if (pk.getClan().getReputationScore() > 0) _clan.takeReputationScore(1);
                            }
                        }
                    }
        ...
        if (getFusionSkill() != null) getCast().stop();
        ...
        if (isPhoenixBlessed()) reviveRequest(this, null, false);
    ...
    public void updateKarmaLoss(long exp) {
        if (!isCursedWeaponEquipped() && getKarma() > 0) {
            ...
            if (karmaLost > 0) setKarma(getKarma() - karmaLost);
        }
    }
    ...
    public void onKillUpdatePvPKarma(Playable target) {
        ...
        // Otherwise, killer is considered as a PKer.
            ...
            if (target instanceof Player) setPkKills(getPkKills() + 1);
    ...
    public void updatePvPStatus() {
        ...
        if (getPvpFlag() == 0) updatePvPFlag(1);
    }
    ...
    public void updatePvPStatus(Creature target) {
        ...
        if (isInDuel() && player.getDuelId() == getDuelId()) return;
    ...
    public void removeMeFromPartyMatch() {
        ...
        if (_partyRoom > 0) {
            ...
            if (room != null) room.removeMember(this);
        }
    }
    ...
    public void giveSkills() {
        if (Config.AUTO_LEARN_SKILLS) rewardSkills();
            ...
            if (getStatus().getLevel() >= 10 && hasSkill(L2Skill.SKILL_LUCKY)) removeSkill(L2Skill.SKILL_LUCKY, false);
    ...
    private void removeInvalidSkills() {
        ...
        for (final L2Skill skill : getSkills().values()) {
            ...
            if (getTemplate().getSkills().stream().filter(s -> s.getId() == skill.getId()).count() == 0) continue;
            ...
            else if (skill.getLevel() > availableSkill.getValue()) addSkill(availableSkill.getSkill(), true);
        }
    }
    ...
    private void regiveTemporarySkills() {
        ...
        if (isNoble()) setNoble(true, false);

        // Add Hero skills if hero.
        if (isHero()) setHero(true);

        // Add Clan skills.
        if (getClan() != null) {
            getClan().checkAndAddClanSkills(this);

            if (getClan().getLevel() >= Config.MINIMUM_CLAN_LEVEL && isClanLeader()) addSiegeSkills();
        }
        ...
        if (_deathPenaltyBuffLevel > 0) addSkill(SkillTable.getInstance().getInfo(5076, _deathPenaltyBuffLevel), false);
    }
    ...
    public List<GeneralSkillNode> getAvailableAutoGetSkills() {
        ...
        getTemplate().getSkills().stream().filter(s -> s.getMinLvl() <= getStatus().getLevel() && s.getCost() == 0).collect(Collectors.groupingBy(s -> s.getId(), Collectors.maxBy(COMPARE_SKILLS_BY_LVL))).forEach((i, s) -> {
            if (getSkillLevel(i) < s.get().getValue()) result.add(s.get());
        });
        return result;
    }
    ...
    public List<GeneralSkillNode> getAvailableSkills() {
        ...
        getTemplate().getSkills().stream().filter(s -> s.getMinLvl() <= getStatus().getLevel() && s.getCost() != 0).forEach(s -> {
            if (getSkillLevel(s.getId()) == s.getValue() - 1) result.add(s);
        });
        return result;
    }
    ...
    public boolean checkAndEquipArrows() {
        ...
        if (arrows.getLocation() == ItemLocation.PAPERDOLL) return true;
    ...
    public boolean disarmWeapon(boolean leftHandIncluded) {
        ...
        if (leftHandIncluded) {
            ...
            if (!ArraysUtil.isEmpty(unequipped)) {
                if (iu == null) iu = new InventoryUpdate();
    ...
    public boolean mount(Summon pet) {
        if (!disarmWeapon(true)) return false;
    ...
    protected class FeedTask implements Runnable {
        ...
        public void run() {
            ...
            if (getCurrentFeed() > getFeedConsume()) setCurrentFeed(getCurrentFeed() - getFeedConsume());
            else {
                ...
                if (wasFlying) teleportTo(TeleportType.TOWN);

                return;
            }

            ItemInstance food = getInventory().getItemByItemId(_petTemplate.getFood1());
            if (food == null) food = getInventory().getItemByItemId(_petTemplate.getFood2());
    ...
    protected synchronized void startFeed(int npcId) {
        ...
        if (_summon != null) {
            ...
            if (!isDead()) _mountFeedTask = ThreadPool.scheduleAtFixedRate(new FeedTask(), 10000, 10000);
        } else if (_canFeed) {
    ...
    public void setAccessLevel(int level) {
        ...
        if (accessLevel.isGm()) {
            ...
            if (!AdminData.getInstance().isRegisteredAsGM(this)) AdminData.getInstance().addGm(this, false);
        } else AdminData.getInstance().deleteGm(this);
        ...
        try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(RESTORE_CHARACTER)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    } else player.setClanPrivileges(Clan.CP_NOTHING);
                    ...
                    if (restoreSubClassData(player) && activeClassId != player.getBaseClass()) {
                        for (final SubClass subClass : player.getSubClasses().values())
                            if (subClass.getClassId() == activeClassId) player._classIndex = subClass.getClassIndex();
                    }
                    ...
                    if (HeroManager.getInstance().isActiveHero(objectId)) player.setHero(true);
    ...
    private static boolean restoreSubClassData(Player player) {
        try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(RESTORE_CHAR_SUBCLASSES)) {
    ...
    private void restoreCharData() {
        ...
        if (!isSubClassActive()) _recipeBook.restore();
    ...
    private void storeCharSub() {
        if (_subClasses.isEmpty()) return;

        try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_CHAR_SUBCLASS)) {
            ...
            try (PreparedStatement ps = con.prepareStatement(ADD_SKILL_SAVE)) {
                ...
                if (storeEffects) {
                    for (final AbstractEffect effect : getAllEffects()) {
                        ...
                        if (effect.getEffectType() == EffectType.HEAL_OVER_TIME) continue;
                        ...
                        if (effect.isHerbEffect() || skill.isToggle()) continue;
    ...
    public int isOnlineInt() {
        if (_isOnline && getClient() != null) return getClient().isDetached() ? 2 : 1;
    ...
    public boolean addSkill(L2Skill newSkill, boolean store, boolean updateShortcuts) {
        ...
        if (oldSkill != null && oldSkill.equals(newSkill)) return false;
        ...
        if (oldSkill != null) {
            ...
            if (oldSkill.triggerAnotherSkill()) removeSkill(oldSkill.getTriggeredId(), false);
        ...
        if (oldSkill != null && getChanceSkills() != null) removeChanceSkill(oldSkill.getId());

        // If new skill got a chance, trigger it.
        if (newSkill.isChance()) addChanceTrigger(newSkill);

        // Add or update the skill in the database.
        if (store) storeSkill(newSkill, -1);
    ...
    public L2Skill removeSkill(int skillId, boolean store, boolean removeEffect) {
        ...
        if (getCast().getCurrentSkill() != null && skillId == getCast().getCurrentSkill().getId()) getCast().stop();
        ...
        if (oldSkill.isChance() && getChanceSkills() != null) removeChanceSkill(skillId);

        if (store) {
            try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(DELETE_SKILL_FROM_CHAR)) {
                ps.setInt(1, skillId);
                ps.setInt(2, getObjectId());
            ...
            if (!oldSkill.isPassive()) getShortcutList().deleteShortcuts(skillId, ShortcutType.SKILL);
        }
        return oldSkill;
    }
    ...
    private void storeSkill(L2Skill skill, int classIndex) {
        try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(ADD_OR_UPDATE_SKILL)) {
            ps.setInt(1, getObjectId());
            ps.setInt(2, skill.getId());
            ps.setInt(3, skill.getLevel());
    ...
    private void restoreSkills() {
        try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(RESTORE_SKILLS_FOR_CHAR)) {
            ps.setInt(1, getObjectId());
            ps.setInt(2, getClassIndex());
    ...
    private void restoreRecom() {
        try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(RESTORE_CHAR_RECOMS)) {
            ps.setInt(1, getObjectId());
    ...
    public boolean isLooterOrInLooterParty(int objectId) {
        if (objectId == getObjectId()) return true;
    ...
    public boolean canCastBeneficialSkillOnPlayable(Playable target, L2Skill skill, boolean isCtrlPressed) {
        ...
        if (targetPlayer.isInsideZone(ZoneId.PVP)) {
            if (isInsideZone(ZoneId.PVP)) return true;

            if (isInsideZone(ZoneId.PEACE)) return isCtrlPressed;
        }
        ...
        if (isInDuel() && targetPlayer.isInDuel() && getDuelId() == targetPlayer.getDuelId()) return true;
        ...
        if (sameParty || sameCommandChannel || sameClan || sameAlliance) return true;

        // If the target not from the same CC/party/alliance/clan is flagged / PK, you can buff with isCtrlPressed.
        if (targetPlayer.getPvpFlag() > 0 || targetPlayer.getKarma() > 0) return isCtrlPressed;
    ...
    public void setMount(int npcId, int npcLevel, int mountType) {
        switch (mountType) {
            case 0: // Dismounted
                if (isFlying()) removeSkill(FrequentSkill.WYVERN_BREATH.getSkill().getId(), false);
            case 1:
    ...
    public void dropAllSummons() {
        ...
        if (_summon != null) _summon.unSummon(this);

        // Delete trained beasts
        if (_tamedBeast != null) _tamedBeast.deleteMe();
    ...
    public void enterObserverMode(ObserverLocation loc) {
        // Adena check.
        if (loc.getCost() > 0 && !reduceAdena("Broadcast", loc.getCost(), this, true)) return;

        dropAllSummons();

        if (getParty() != null) getParty().removePartyMember(this, MessageType.EXPELLED);
    ...
    public void enterOlympiadObserverMode(int id) {
        ...
        if (!isInObserverMode()) _savedLocation.set(getPosition());
    ...
    public boolean canDuel() {
        ...
        else if (isInDuel()) _noDuelReason = SystemMessageId.S1_CANNOT_DUEL_BECAUSE_S1_IS_ALREADY_ENGAGED_IN_A_DUEL;
    ...
    public boolean addSubClass(int classId, int classIndex) {
        ...
        try {
            if (_subClasses.size() == 3 || classIndex == 0 || _subClasses.containsKey(classIndex)) return false;

            final SubClass subclass = new SubClass(classId, classIndex);

            try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(ADD_CHAR_SUBCLASS)) {
                ps.setInt(1, getObjectId());
    ...
    public boolean setActiveClass(int classIndex) {
        ...
        try {
            ...
            if (_party != null) _party.recalculateLevel();

            if (_summon instanceof Servitor) _summon.unSummon(this);
            ...
            if (isSubClassActive()) _recipeBook.clear();
            else _recipeBook.restore();
            ...
            if (getStatus().getHp() > max) getStatus().setHp(max);

            max = getStatus().getMaxMp();
            if (getStatus().getMp() > max) getStatus().setMp(max);

            max = getStatus().getMaxCp();
            if (getStatus().getCp() > max) getStatus().setCp(max);
            ...
            final ItemInstance item = getActiveWeaponInstance();
            if (item != null) item.unChargeAllShots();
    ...
    public void onPlayerEnter() {
        ...
        if (isGM()) {
            ...
            if (!getAppearance().isVisible()) sendMessage("Entering world in Invisible mode.");

            if (getBlockList().isBlockingAll()) sendMessage("Entering world in Refusal mode.");
        }
    ...
    public void reviveRequest(Player reviver, L2Skill skill, boolean isPet) {
        if (_reviveRequested == 1) {
            ...
            if (_revivePet == isPet) reviver.sendPacket(SystemMessageId.RES_HAS_ALREADY_BEEN_PROPOSED);
            else {
                if (isPet)
    ...
    public void reviveAnswer(int answer) {
        ...
        if (answer == 0 && isPhoenixBlessed()) stopPhoenixBlessing(null);
            ...
            if (!_revivePet) {
                ...
                if (_revivePower != 0) _summon.doRevive(_revivePower);
                else _summon.doRevive();
            }
        }
        _reviveRequested = 0;
        _revivePower = 0;
    ...
    public final void onTeleported() {
        ...
        if (Config.PLAYER_SPAWN_PROTECTION > 0) setSpawnProtection(true);

        // Modify the position of the tamed beast if necessary
        if (_tamedBeast != null) _tamedBeast.teleportTo(getPosition(), 0);

        // Modify the position of the pet if necessary
        if (_summon != null) _summon.teleportTo(getPosition(), 0);

        // If under shop mode, cancel it. Leave the Player sat down.
        if (isInStoreMode()) setOperateType(OperateType.NONE);
    }
    ...
    public void addExpAndSp(long addToExp, int addToSp, Creature creature) {
        getStatus().addExpAndSp(addToExp, addToSp, creature);
    }
    ...
    public void addExpAndSp(long addToExp, int addToSp, Map<Creature, RewardInfo> rewards, Creature creature) {
        getStatus().addExpAndSp(addToExp, addToSp, rewards, creature);
    }
    ...
    public ItemInstance validateItemManipulation(int objectId) {
        ...
        if (CursedWeaponManager.getInstance().isCursed(item.getItemId())) return null;
    ...
    private synchronized void cleanup() {
        try {
            ...
            else if (_summon != null) _summon.unSummon(this);
            ...
            if (_party != null) _party.removePartyMember(this, MessageType.DISCONNECTED);
            ...
            if (isInObserverMode()) setXYZInvisible(_savedLocation);
            ...
            if (_clan != null) _clan.broadcastToMembersExcept(this, new PledgeShowMemberListUpdate(this));

            if (isSeated()) {
                final WorldObject object = World.getInstance().getObject(_throneId);
                if (object instanceof StaticObject) ((StaticObject) object).setBusy(false);
            }
    ...
    public void calculateDeathPenaltyBuffLevel(Creature killer) {
        ...
        if ((getKarma() > 0 || Rnd.get(1, 100) <= Config.DEATH_PENALTY_CHANCE) && !(killer instanceof Player) && !(getCharmOfLuck() && (killer == null || killer.isRaidRelated())) && !isPhoenixBlessed() && !(isInsideZone(ZoneId.PVP) || isInsideZone(ZoneId.SIEGE))) {
            if (_deathPenaltyBuffLevel != 0) removeSkill(5076, false);
    ...
    public void checkItemRestriction() {
        for (final ItemInstance item : getInventory().getPaperdollItems()) {
            if (item.getItem().checkCondition(this, this, false)) continue;
    ...
    public void enterOnNoLandingZone() {
        if (getMountType() == 2) {
            if (_dismountTask == null) _dismountTask = ThreadPool.schedule(this::dismount, 5000);
    ...
    public final boolean isFalling(int z) {
        if (isDead() || getMove().getMoveType() != MoveType.GROUND) return false;

        if (System.currentTimeMillis() < _fallingTimestamp) return true;

        final int deltaZ = getZ() - z;
        if (deltaZ <= getBaseTemplate().getSafeFallHeight(getAppearance().getSex())) return false;
    ...
    public boolean isAllowedToEnchantSkills() {
        ...
        if (getCast().isCastingNow()) return false;

        if (isInBoat()) return false;
    ...
    public void sendInfo(Player player) {
        if (_boat != null) getPosition().set(_boat.getPosition());
        ...
        if (player.getSummon() != null) sendPacket(new RelationChanged(player.getSummon(), relation, isAutoAttackable));

        if (_boat != null) player.sendPacket(new GetOnVehicle(getObjectId(), _boat.getObjectId(), getBoatPosition()));
    ...
    public boolean teleportRequest(Player requester, L2Skill skill) {
        if (_summonTargetRequest != null && requester != null) return false;
    ...
    public void teleportAnswer(int answer, int requesterId) {
        if (_summonTargetRequest == null) return;
    ...
    public void activateGate(int answer, int type) {
        ...
        if (answer == 1 && getTarget() == _requestedGate && _requestedGate.getClanHall() != null && getClan() != null && getClanId() == _requestedGate.getClanHall().getOwnerId() && hasClanPrivileges(Clan.CP_CH_OPEN_DOOR)) {
            if (type == 1) _requestedGate.openMe();
            else if (type == 0) _requestedGate.closeMe();
        }
    ...
    public final void refreshInfos() {
        for (final WorldObject object : getKnownType(WorldObject.class)) {
            if (object instanceof Player && ((Player) object).isInObserverMode()) continue;
    ...
    public final void teleportTo(Location loc, int randomOffset) {
        if (DimensionalRiftManager.getInstance().checkIfInRiftZone(getX(), getY(), getZ(), true)) {
            if (isInParty() && getParty().isInDimensionalRift()) getParty().getDimensionalRift().usedTeleport(this);
    ...
    public final void stopFakeDeath(boolean removeEffects) {
        if (removeEffects) stopEffects(EffectType.FAKE_DEATH);
    ...
    public boolean isMystic() {
        return isMageClass() && getClassId() != ClassId.ORC_MYSTIC && getClassId() != ClassId.ORC_SHAMAN;
    }
}