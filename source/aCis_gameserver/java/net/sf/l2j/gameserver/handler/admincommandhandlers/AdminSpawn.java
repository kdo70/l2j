...
public class AdminSpawn implements IAdminCommandHandler {
    ...
    public void useAdminCommand(String command, Player player) {
        if (command.startsWith("admin_list_spawns")) {
            ...
            final Spawn spawnTable = targetNpc.getSpawnTable();
            final ASpawn spawn = targetNpc.getSpawn();
            if (spawn != null) {
                SpawnTable.getInstance().deleteSpawn(spawnTable, true);
        ...
        } else if (command.startsWith("admin_replace")) {
            final WorldObject targetWorldObject = player.getTarget();
            if (!(targetWorldObject instanceof Npc targetNpc)) {
                player.sendPacket(SystemMessageId.INVALID_TARGET);
                return;
            ...
            if (spawn != null) {
                ...
                spawn(player, Integer.toString(id), 60, true);
            }
