...
public final class Formulas {
    ...
    public static double calcBlowDamage(Creature attacker, Creature target, L2Skill skill, ShieldDefense sDef, boolean ss) {
        ...
        double pveMul = 1.;
        if (!isPvP && !target.isRaidBoss()) {
            double monsterMulPhysDmg = attacker.getStatus().calcStat(Stats.MONSTER_MUL_PHYS_DMG, 0, null, null);
            if (monsterMulPhysDmg != 0) {
                if (monsterMulPhysDmg > 1) {
                    monsterMulPhysDmg -= 1;
                }
                pveMul += monsterMulPhysDmg;
            }
        }
        ...
        if (!isPvP && !target.isRaidBoss()) {
            double monsterMulPhysDef = target.getStatus().calcStat(Stats.MONSTER_MUL_PHYS_DEF, 0, null, null);
            if (monsterMulPhysDef != 0) {
                damage /= monsterMulPhysDef;
            }
        }
    ...
    public static final double calcPhysicalAttackDamage(Creature attacker, Creature target, ShieldDefense sDef, boolean crit, boolean ss) {
        ...
        double damage = 0;
        if (crit)
            damage = ((attackPower * 2. * critDamMul * critDamPosMul * critVuln * posMul * rndMul * raceMul * pvpMul * pveMul * elemMul * weaponMul) + addCritPower) * 77. / defence;
        else
            damage = (attackPower * posMul * rndMul * raceMul * pvpMul * pveMul * elemMul * weaponMul) * 77. / defence;
    ...
    public static final double calcPhysicalSkillDamage(Creature attacker, Creature target, L2Skill skill, ShieldDefense sDef, boolean crit, boolean ss) {
        ...
        // End calculation.
        double damage = ((attackPower + skillPower) * ssMul * rndMul * raceMul * pvpMul * pveMul * elemMul * weaponMul) * 77. / defence;
        if (crit)
            damage *= 2.;
    ...
    public static final double calcMagicDam(Creature attacker, Creature target, L2Skill skill, ShieldDefense sDef, boolean ss, boolean bss, boolean mcrit) {
        ...
        if (!isPvP && !target.isRaidBoss()) {
            double monsterMulMagicDmg = attacker.getStatus().calcStat(Stats.MONSTER_MUL_MAGIC_DMG, 0, null, null);
            if (monsterMulMagicDmg != 0) {
                damage *= monsterMulMagicDmg;
            }

            double monsterMulMagicDef = target.getStatus().calcStat(Stats.MONSTER_MUL_MAGIC_DEF, 0, null, null);
            if (monsterMulMagicDef != 0) {
                damage /= monsterMulMagicDef;
            }
        }
